...
For a sequence of complex numbers $\{z_i\}_{i=1}^{n}$, if for any $i$, the condition $\operatorname{Arg}(z_i)\in (-\pi/2, \pi/2)$ holds, where $\operatorname{Arg}(z)\in(-\pi, \pi]$ represents the principal argument of a complex number $z$, then the following inequality is true:
$$
\operatorname{Arg}\left(\sum_{i=1}^{n}z_i\right) \in \left[\min_i\{\operatorname{Arg}(z_i)\}, \max_i\{\operatorname{Arg}(z_i)\}\right].
$$
...
$$
\operatorname{Arg}(z_1+z_2) = \arctan\left(\frac{r_1\sin\varphi_1+r_2\sin\varphi_2}{r_1\cos\varphi_1+r_2\cos\varphi_2}\right), 
$$
...
$$
\frac{\partial}{\partial r_1}\operatorname{Arg}(z_1+z_2) > 0 \quad \text{and} \quad \frac{\partial}{\partial r_2}\operatorname{Arg}(z_1+z_2) < 0.
$$
Consequently, the argument $\operatorname{Arg}(z_1+z_2)$ reaches its maximum and minimum values at the boundaries of the domain of $r_1, r_2$:
$$
\begin{aligned}
\max \operatorname{Arg}(z_1+z_2) &= \lim_{\substack{r_1 \to +\infty \\ r_2 \to 0^+}}\operatorname{Arg}(z_1+z_2) = \varphi_1, \\
\min \operatorname{Arg}(z_1+z_2) &= \lim_{\substack{r_1 \to 0^+ \\ r_2 \to +\infty}}\operatorname{Arg}(z_1+z_2) = \varphi_2.
\end{aligned}
$$
Therefore, for $n=2$, we have shown:
$$
\operatorname{Arg}(z_1+z_2) \in [\min\{\varphi_1,\varphi_2\}, \max\{\varphi_1,\varphi_2\}].
$$
For the general case of $n$ complex numbers, we can apply this result inductively:
$$
\operatorname{Arg}\left(\sum_{i=1}^{n}z_i\right) = \operatorname{Arg}\left(z_n+\sum_{i=1}^{n-1}z_i\right) \in \left[\min\left\{\varphi_n, \operatorname{Arg}\left(\sum_{i=1}^{n-1}z_i\right)\right\}, \max\left\{\varphi_n, \operatorname{Arg}\left(\sum_{i=1}^{n-1}z_i\right)\right\}\right].
$$
...
The yaw angle $\varphi(\mathbf{l})$ of a vector $\mathbf{l}$ is defined counter-clockwise from the positive longitudinal axis. We map vectors to the complex domain by aligning the real axis with the positive longitudinal axis and the imaginary axis with the negative radial axis. This specific mapping ensures that a complex number $z$'s argument $\operatorname{Arg}(z)$ directly corresponds to its vector's yaw angle.

We identify the vector $\mathbf{v}_i = \mathbf{P}_{i+1}-\mathbf{P}_{i}$ with a complex number $z_i$. As per the action space design, the longitudinal coordinates of the control points are strictly increasing. Consequently, $\mathbf{v}_i$ lies strictly within the upper half-plane, which implies that $\operatorname{Re}(z_i)>0$. Therefore, $\operatorname{Arg}(z_i) \in (-\pi/2, \pi/2)$.
...
&= \operatorname{Arg}\left(n\sum_{i=0}^{n-1}z_i b_{i,n-1}(\tau)\right) \\
&\in \left[\min_{i}\{\operatorname{Arg}(n z_i b_{i,n-1}(\tau))\}, \max_{i}\{\operatorname{Arg}(n z_i b_{i,n-1}(\tau))\}\right] \\
&= \left[\min_{i}\{\operatorname{Arg}(z_i)\}, \max_{i}\{\operatorname{Arg}(z_i)\}\right] \\
